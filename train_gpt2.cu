...
// ----------------------------------------------------------------------------
// gen_tiktoken_script via python

void gen_tiktoken_script(int step, int* gen_tokens, int gen_max_length)
{
    char fn_tiktoken_py[32];
    const char
        *tiktoken_py_1 =
                "import tiktoken\n"
                "import tiktoken\n"
    ...
    snprintf(fn_tiktoken_py, sizeof fn_tiktoken_py, "tiktoken_step_%d.py", step);
    FILE *f = fopen(fn_tiktoken_py, "w");
    if (f) {
        char tiktoken_cmd[40];
        ...
        snprintf(tiktoken_cmd, sizeof tiktoken_cmd, "python %s", fn_tiktoken_py);
        printf("(run '%s' to see the output as words)\n", tiktoken_cmd);
        // uncmment the line below to actually run that now
        // system(tiktoken_cmd);
    }
}
...
int main() {
    ...
    for (int step = 0; step <= 40; step++) {
        ...
        if (step > 0 && step % 20 == 0) {
            ...
            // hacky output of tokens as words
            gen_tiktoken_script(step, gen_tokens, gen_max_length);
        }
