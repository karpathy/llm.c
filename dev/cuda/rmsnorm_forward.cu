...
void rmsnorm_forward_cpu(
    ...
    for (int b = 0; b < B; b++) {
        for (int t = 0; t < T; t++) {
            ...
            float rms_val = 1.0f / rsqrtf(sum_of_squares / C);
...
__global__ void rmsnorm_forward_kernel1(
    ...
    if (idx < N) {
        ...
        float rms_val = 1.0f / rsqrtf(sum_of_squares);
...
void rmsnorm_forward1(
    ...
    rms_forward_kernel1<<<grid_size, block_size>>>(out, rms, inp, weight, bias, N, C);
...
int main(int argc, char **argv) {
    ...
    for (int j = 0; j < sizeof(block_sizes) / sizeof(int); j++) {
        ...
        layernorm_forward(kernel_num, d_out, d_rms, d_inp, d_weight, d_bias, B, T, C, block_size);
