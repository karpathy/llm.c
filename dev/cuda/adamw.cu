...
// kernel with overloaded operators for float4
...
__device__ inline float4 operator/(const float4& a, float b) {
...
__device__ inline float4 operator*(float a, const float4& b) {
...
__device__ inline float4 lerp(const float4& start, const float4& end, float weight) {
    return make_float4(lerp(start.x, end.x,  weight), lerp(start.y, end.y, weight), lerp(start.z, end.z, weight), lerp(start.w, end.w, weight));
}

__global__ void adamw_kernel4(float4* params_memory, const float4* grads_memory, float4* m_memory, float4* v_memory, long num_parameters,
                              float learning_rate, float beta1, float beta2, float beta1_correction, float beta2_correction, float eps, float weight_decay) {
   int i = blockIdx.x * blockDim.x + threadIdx.x;
   assert(num_parameters % 4 == 0);
   if (i >= num_parameters / 4) return;  // guard
...
// version 4: using overloaded operators for float4
void adamw_dispatch4(float* params_memory, const float* grads_memory, float* m_memory, float* v_memory, long num_parameters,
                     float learning_rate, float beta1, float beta2, float beta1_correction, float beta2_correction, float eps, float weight_decay) {
    unsigned int block_size = 512;
    unsigned int num_blocks = ceil_div(num_parameters, (long) block_size);
    adamw_kernel4<<<num_blocks, block_size>>>((float4*) params_memory, (float4*) grads_memory, (float4*) m_memory, (float4*) v_memory, num_parameters,
                                              learning_rate, beta1, beta2, beta1_correction, beta2_correction, eps, weight_decay);
    cudaCheck(cudaGetLastError());
}
