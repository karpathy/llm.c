...
Will save reference tensors to disk for checking correctness w.r.t. C implementation.
...
from typing import AbstractSet, Collection, Dict, Iterator, List, Tuple, Literal, Optional, Sequence, Union, cast
...
# Llama 3 replacement for LayerNorm: no bias, and normalizes by Norm instead of StdDev
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, freqs_cis=None, start_pos=None):
        ...
        qkv = self.c_attn(x)
        # split out the qkv into 3 separate tensors and break up the last dim into (n_head, head_dim)
        ...
        # Grouped Query Attention (GQA) <-- 2. difference compared to GPT-2
        k = repeat_kv(k, self.n_rep)
        v = repeat_kv(v, self.n_rep)
        # move head dim forward to be a batch dim, as we're about to apply attention
        ...
        # concatenate all the head outputs together channel-wise
...
class LLaMA(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.init_rng = torch.Generator() # let's be very explicit with our rng
    ...
    def adapt_llama_state_dict_keys(checkpoint, config: LlamaConfig):
        # convert from Meta official Llama release state_dict keys to our own llm.c style naming
        # that is more consistent with GPT-2. Just done for convenience.
    ...
    def adapt_llama_state_dict_keys_hf(checkpoint, config: LlamaConfig):
        # convert from HuggingFace Llama checkpoint state_dict keys to our own llm.c style naming
        # that is more consistent with GPT-2. Just done for convenience.
        # HuggingFace permuted the weights around a bit, so we need to unpermute them
...
def sample_top_p(probs, p):
    # sample topp along the last axis of tensor probs, p \in (0, 1]
