...
__global__ void fused_rmsnorm_residual_forward_kernel5(floatX* residual, floatX* normed, float* rrms,
                                               const floatX* inp1, const floatX* inp2,
                                               const floatX* weight,
                                               int N, int C) {
    // this kernel is a simplified version of fused_residual_forward_kernel5 
    assert(blockDim.x == WARP_SIZE);
...
__global__ void __launch_bounds__(512, 2) // todo - any warnings on Turing with only 1024 threads?
    rmsnorm_backward_kernel10(floatX* dinp, floatX* dweight, float* scratch,
                                ...
                                const float* rstd,
                                int B, int T, int C) {
    // this is the same as layernorm_backward_kernel10 just simplified
    int BLOCK_SIZE = blockDim.x;
    int warpsInBlock = BLOCK_SIZE / WARP_SIZE; // number of warps in block
    extern __shared__ float shared[];
    ...
    // the obvious solution is to change the addressing below to use (threadId.x-32) as offset, but that causes
    // register spills, so instead we mess with the base pointer here, which doesn't increase register usage.
    float* dweight_tmp_shared = shared + 2 * rounded_C - WARP_SIZE * f128::size;  // TODO(gordicaleksa): check if this is correct, do we need BLOCK_SIZE * f128::size?
...
void fused_rmsnorm_residual_forward5(floatX* residual, floatX* normed, float* rstd,
                             const floatX* inp1, const floatX* inp2,
                             const floatX* weight,
                             int N, int C, cudaStream_t stream) {
    const int block_size = 256;
    ...
    // this may fail, in which case we fall back to the smem free implementation.
    cudaCheck(cudaGetLastError());
    auto status = cudaFuncSetAttribute(fused_rmsnorm_residual_forward_kernel5, cudaFuncAttributeMaxDynamicSharedMemorySize, smem);
    cudaCheck(cudaGetLastError());
    if(status == cudaSuccess) {
        fused_rmsnorm_residual_forward_kernel5<<<grid_size, dim3(WARP_SIZE, block_y), smem, stream>>>(residual, normed,
                                                                                              rstd, inp1, inp2,
                                                                                              weight, N, C);
    } else {
        residual_forward(residual, inp1, inp2, N*C, stream);
        rmsnorm_forward(normed, rstd, residual, weight, N, 1, C, stream);
    }
    cudaCheck(cudaGetLastError());
}
...
void rmsnorm_backward(floatX* dinp, floatX* dweight, float* scratch,
                        const floatX* dout, const floatX* inp, const floatX* weight, const float* rrms,
                        int B, int T, int C, cudaStream_t stream) {
    NVTX_RANGE_FN();
    const int block_size = 512;
    const int blocks_per_sm = 2; // supported on every architecture and less cache thrashing than 3
    const int grid_size = blocks_per_sm * deviceProp.multiProcessorCount;
    size_t rounded_C = CEIL_DIV(C, (32 * x128::size)) * (32 * x128::size);
    size_t shared_mem_size = (1 * rounded_C + 1 * (block_size - 32) * f128::size) * sizeof(float);

    cudaCheck(cudaMemsetAsync(scratch, 0, 1 * sizeof(float), stream)); // only need to reset the flag to 0
    rmsnorm_backward_kernel10<<<grid_size, block_size, shared_mem_size, stream>>>(dinp, dweight, scratch, dout, inp, weight, rrms, B, T, C);
    cudaCheck(cudaGetLastError());
}
