...
void set_vector(float *data, int N, float value,
...
typedef struct dpct_type_405628 {
...
MultiGpuConfig multi_gpu_config_init(int *argc, char ***argv) {
    ...
    /*
    DPCT1093:350: The "result.local_device_idx" device may be not the one
    intended for use. Adjust the selected device if needed.
    */
    cudaCheck(DPCT_CHECK_ERROR(dpct::select_device(result.local_device_idx)));
...
int main(int argc, char **argv) {
  ...
  */
    dpct::get_in_order_queue().submit([&](sycl::handler &cgh) {
        float multi_gpu_config_process_rank_ct2 =
            (float)(multi_gpu_config.process_rank + 1);

        cgh.parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, n_blocks) *
                                  sycl::range<3>(1, 1, threads_per_block),
                              sycl::range<3>(1, 1, threads_per_block)),
            [=](sycl::nd_item<3> item_ct1) {
                set_vector(all_reduce_buffer, all_reduce_buffer_size,
                           multi_gpu_config_process_rank_ct2, item_ct1);
            });
    });
  /*
  DPCT1010:351: SYCL uses exceptions to report errors and does not use the error
  ...
  */
  ...
  cudaCheck(
      DPCT_CHECK_ERROR(dpct::get_in_order_queue()
                           .memcpy(all_reduce_buffer_host, all_reduce_buffer,
  ...
  cudaCheck(DPCT_CHECK_ERROR(dpct::get_in_order_queue()
                                 .memcpy(all_reduce_buffer_host,
                                         all_reduce_buffer_recv,
                                         sizeof(float) * all_reduce_buffer_size)
                                 .wait()));
  ...
  cudaCheck(DPCT_CHECK_ERROR(
      dpct::dpct_free(all_reduce_buffer, dpct::get_in_order_queue())));
