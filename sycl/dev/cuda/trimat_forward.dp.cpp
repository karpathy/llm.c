...
SYCL_EXTERNAL void permute_kernel(float *q, float *k, float *v,
                                  const float *inp, int B, int T, int NH,
                                  int HS, const sycl::nd_item<3> &item_ct1) {
...
void trimul_cublas(float* preatt,
    ...
    /*
    DPCT1010:469: SYCL uses exceptions to report errors and does not use the
...
void trimul_global(float* out, const float* inp, int T, int C, int NH,
                                             const sycl::nd_item<3> &item_ct1,
                                             sycl::local_accessor<float, 2> lhs_s,
                                             sycl::local_accessor<float, 2> rhs_s) {
    ...
    int b = item_ct1.get_group(0) / NH;
    int nh = item_ct1.get_group(0) % NH;
    ...
    matmul_tri(r, T, k, C3, q, C3, T, HS, scale, item_ct1, lhs_s, rhs_s);
}
...
void trimul_launcher(float* out, const float* inp, int B, int T, int C, int NH) {
    ...
    dpct::get_in_order_queue().submit([&](sycl::handler &cgh) {
        sycl::local_accessor<float, 2> lhs_s_acc_ct1(sycl::range<2>(128, 32),
                                                     cgh);
        sycl::local_accessor<float, 2> rhs_s_acc_ct1(sycl::range<2>(128, 32),
                                                     cgh);

        cgh.parallel_for(
            sycl::nd_range<3>(sycl::range<3>(NH * B, T / 128, T / 128) *
                                  sycl::range<3>(1, 16, 16),
                              sycl::range<3>(1, 16, 16)),
            [=](sycl::nd_item<3> item_ct1) {
                trimul_global<matmul_tri>(out, inp, T, C, NH, item_ct1,
                                          lhs_s_acc_ct1, rhs_s_acc_ct1);
            });
    });
    /*
    DPCT1010:470: SYCL uses exceptions to report errors and does not use the
    error codes. The call was replaced with 0. You need to rewrite this code.
    */
    cudaCheck(0);
}
...
void matmul_tri_naive(float* p, int PS, const float* k, int KS, const float* q, int QS, int T, int HS, float alpha,
                      const sycl::nd_item<3> &item_ct1) {
    // coordinate system:
    // | - - - - - > j
    // |
    ...
    int i_base = 128 * item_ct1.get_group(2) + 8 * item_ct1.get_local_id(2);
    int j_base = 128 * item_ct1.get_group(1) + 8 * item_ct1.get_local_id(1);
...
/*
DPCT1110:111: The total declared local variable size in device function
matmul_tri_registers exceeds 128 bytes and may cause high register pressure.
Consult with your hardware vendor to find the total register size available and
adjust the code, or use smaller sub-group size to avoid high register pressure.
*/
void matmul_tri_registers(float *p, int PS, const float *k, int KS,
                          const float *q, int QS, int T, int HS, float alpha,
                          const sycl::nd_item<3> &item_ct1) {
    int i_base = 128 * item_ct1.get_group(2) + 8 * item_ct1.get_local_id(2);
    int j_base = 128 * item_ct1.get_group(1) + 8 * item_ct1.get_local_id(1);
...
/*
DPCT1110:112: The total declared local variable size in device function
matmul_tri3 exceeds 128 bytes and may cause high register pressure. Consult with
...
*/
void matmul_tri3(float *p, int PS, const float *k, int KS, const float *q,
                 int QS, int T, int HS, float alpha,
                 const sycl::nd_item<3> &item_ct1) {
...
matmul_tri4 exceeds 128 bytes and may cause high register pressure. Consult with
your hardware vendor to find the total register size available and adjust the
code, or use smaller sub-group size to avoid high register pressure.
*/
void matmul_tri4(float *p, int PS, const float *k, int KS, const float *q,
                 int QS, int T, int HS, float alpha,
                 const sycl::nd_item<3> &item_ct1,
                 sycl::local_accessor<float, 2> lhs_s,
                 sycl::local_accessor<float, 2> rhs_s) {
    ...
    q += 128 * item_ct1.get_group(2) * QS;
    k += 128 * item_ct1.get_group(1) * KS;
    ...
    for (int so = 0; so < HS; so += 32) {
        ...
        */
        item_ct1.barrier();
        for (int y = item_ct1.get_local_id(1) / 2; y < 128; y += 8) {
            int xo = (item_ct1.get_local_id(1) % 2) * 16;
            lhs_s[y][item_ct1.get_local_id(2) + xo] =
                q[y * QS + so + item_ct1.get_local_id(2) + xo];
            rhs_s[y][item_ct1.get_local_id(2) + xo] =
                k[y * KS + so + item_ct1.get_local_id(2) + xo];
        }
        /*
        ...
        */
        /*
        DPCT1065:472: Consider replacing sycl::nd_item::barrier() with
        sycl::nd_item::barrier(sycl::access::fence_space::local_space) for
        better performance if there is no access to global memory.
        */
        item_ct1.barrier();
        ...
        for (int si = 0; si < 32; ++si) {
            float rhs[8];
            for (int u = 0; u < 8; ++u) {
                rhs[u] = rhs_s[u + 8 * item_ct1.get_local_id(1)]
                              [(si + item_ct1.get_local_id(2)) % 32];
            }

            for (int ii = 0; ii < 8; ++ii) {
                float lhs = lhs_s[ii + 8 * item_ct1.get_local_id(2)]
                                 [(si + item_ct1.get_local_id(2)) % 32];
...
int main(int argc, char **argv) {
    ...
    float* d_inp;
    cudaCheck(
        DPCT_CHECK_ERROR(d_out = sycl::malloc_device<float>(
