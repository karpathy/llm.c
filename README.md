...
Prepare the GPT-2 decoding vocab from `tiktoken`, it will create `data/decode_gpt2.txt` which maintains the mapping between token ids (`line number - 1`) and tokens:

```bash
python prepro_decoder.py
```
